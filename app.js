...
(async () => {
  ...
  async function doAutomationCancelListPropDevProposals(browser, leftPortionOfKRDirectLinkToModule, recordNumsToUpdateInKRArr, isKrUsingNewDashboardWithIframes, pathOfScreenshotDir) {
    ...
    await tryCancellingSinglePropDevProposalCaptureScreenshotOnException(0, browser, leftPortionOfKRDirectLinkToModule, recordNumsToUpdateInKRArr, isKrUsingNewDashboardWithIframes, pathOfScreenshotDir);
    await tryCancellingSinglePropDevProposalCaptureScreenshotOnException(1, browser, leftPortionOfKRDirectLinkToModule, recordNumsToUpdateInKRArr, isKrUsingNewDashboardWithIframes, pathOfScreenshotDir);
    await tryCancellingSinglePropDevProposalCaptureScreenshotOnException(2, browser, leftPortionOfKRDirectLinkToModule, recordNumsToUpdateInKRArr, isKrUsingNewDashboardWithIframes, pathOfScreenshotDir);
   ...
   * @param {number}   index    The array index (position) in the config json file array of KR records to be automated, allows this function to be called for a long list of KR records in that array
   * @param {Object}   browser    The main Puppeteer browser object, will be needed to inspect the current list of iframes and potentially open new tabs
   * @param {string}   leftPortionOfKRDirectLinkToModule    The left portion of the direct link to a KR prop dev proposal, minus the proposal number portion, as would be generated from the link pop up at the top of KR prop dev module screen
   * @param {number[]} recordNumsToUpdateInKRArr    An array of the Prop Dev numbers that are used in KR as the rightmost portion of the direct link generated by the link feature of the Prop Dev module to generate a direct link/URL to pull up a specific Prop Dev record
   * @param {boolean}  krUsingNewDashboardWithIframes   Flag that indicates whether the KR dashboard is curently enabled
   ...
   * 
   */
async function tryCancellingSinglePropDevProposalCaptureScreenshotOnException(index, browser, leftPortionOfKRDirectLinkToModule, recordNumsToUpdateInKRArr, isKrUsingNewDashboardWithIframes, pathOfScreenshotDir) {
  const currPropDevDirectLink = leftPortionOfKRDirectLinkToModule + recordNumsToUpdateInKRArr[index];
  try {
    await automateCancellingSinglePropDevProposal(browser, currPropDevDirectLink, isKrUsingNewDashboardWithIframes, pathOfScreenshotDir);
  } catch (e) {
    // statements to handle any exceptions
    console.error(`automateCancellingSinglePropDevProposal of ${recordNumsToUpdateInKRArr[index]} failed with exception ${JSON.stringify(e)}`);
    const pageTab1 = (await browser.pages())[0];
    takeScreenshot(pageTab1, `exceptionOnPD${recordNumsToUpdateInKRArr[index]}`, currPropDevDirectLink, pathOfScreenshotDir);      
  }  
}
